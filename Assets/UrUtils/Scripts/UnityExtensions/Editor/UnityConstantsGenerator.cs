    ...
    public static class UnityConstantsGenerator
    {
        ...
        public static void Generate()
        {
            ...
            using (var writer = new StreamWriter(filePath))
            {
                ...
                // Write out sorting layers as enum
                if (sortingLayerNames != null)
                {
                    writer.WriteLine("    public enum SortingLayersEnum");
                    writer.WriteLine("    {");
                    for (int i = 0; i < sortingLayerNames.Length; i++)
                    {
                        var name = sortingLayerNames[i];
                        ...
                        writer.WriteLine("        /// ID of sorting layer '{0}'.", name);
                        writer.WriteLine("        /// </summary>");
                        writer.WriteLine("        {0} = {1},", MakeSafeForCode(name), id);
                    }
                    writer.WriteLine("    };");
                    writer.WriteLine();
                }
                ...
                // Write out enables scenes
                writer.WriteLine("    public static class ScenesEnabled");
                writer.WriteLine("    {");
                int sceneIndex = 0;
                int disabledIndex = -1;
                for (int i = 0; i < EditorBuildSettings.scenes.Length; i++)
                {
                    ...
                    writer.WriteLine("        /// <summary>");
                    writer.WriteLine("        /// Name of '{0}'.", sceneName);
                    writer.WriteLine("        /// </summary>");
                    writer.WriteLine("        public const int {0} = {1};", MakeSafeForCode(sceneName), scene.enabled ? sceneIndex++ : disabledIndex--);
                }
                writer.WriteLine("    }");
                writer.WriteLine("}");
                writer.WriteLine();
